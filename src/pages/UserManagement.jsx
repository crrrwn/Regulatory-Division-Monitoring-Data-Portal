...
export default function UserManagement() {
  ...
  const [accessAllowedSections, setAccessAllowedSections] = useState([])
  ...
  const staffUsers = users.filter((u) => (u.role || 'staff') === 'staff')
  const selectedAccessUser = staffUsers.find((u) => u.id === accessUserId)

  useEffect(() => {
    if (!selectedAccessUser) {
      setAccessAllowedSections([])
      return
    }
    const allowed = Array.isArray(selectedAccessUser.allowedSections) && selectedAccessUser.allowedSections.length > 0
      ? selectedAccessUser.allowedSections
      : selectedAccessUser.section
        ? [selectedAccessUser.section]
        : []
    setAccessAllowedSections(allowed)
  }, [selectedAccessUser])
  ...
  const toggleAccessSection = (sectionId) => {
    setAccessAllowedSections((prev) =>
      prev.includes(sectionId) ? prev.filter((s) => s !== sectionId) : [...prev, sectionId],
    )
  }

  const saveAccessSections = async () => {
    if (!accessUserId) return
    setAccessSaving(true)
    try {
      await updateDoc(doc(db, 'users', accessUserId), {
        allowedSections: accessAllowedSections,
        updatedAt: new Date().toISOString(),
      })
      setUsers((prev) =>
        prev.map((u) =>
          u.id === accessUserId ? { ...u, allowedSections: accessAllowedSections } : u,
        ),
      )
      await addSystemLog({
        ...
        role: 'admin',
        details: `Section access updated for user ${selectedAccessUser?.email}.`,
      }).catch(() => {})
      showNotification({
        type: 'success',
      ...
      showNotification({
        type: 'error',
        title: 'Update failed',
        message: err.message || 'Could not save sections.',
        toast: true,
      })
    }
  ...
  return (
    <div className="space-y-6 pb-10 min-w-0 w-full max-w-full overflow-x-hidden">
      ...
      <div className="user-mgmt-anim-4 rounded-2xl border-2 border-[#e8e0d4] bg-white shadow-lg shadow-[#1e4d2b]/8 overflow-hidden hover:shadow-xl hover:shadow-[#1e4d2b]/12 transition-all duration-500 ease-[cubic-bezier(0.33,1,0.68,1)] flex flex-col">
        <div className="shrink-0 bg-gradient-to-r from-[#1e4d2b] via-[#1a4526] to-[#153019] px-5 py-3.5 relative overflow-hidden border-b-2 border-[#1e4d2b]/20">
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold text-white/80 tracking-wider mt-0.5">
                Select a staff user and choose which sections they can access
              </p>
            </div>
          </div>
        </div>
        <div className="p-5 sm:p-6 flex-1 bg-gradient-to-b from-[#faf8f5] to-[#f2ede6] border-l-4 border-[#1e4d2b]/25">
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-[#5c7355] uppercase tracking-wider mb-1.5">
                Select User (by section)
              </label>
              <AppSelect
                value={accessUserId}
                onChange={(v) => setAccessUserId(v || '')}
                groups={(() => {
                  const placeholder = [{ sectionLabel: '—', options: [{ value: '', label: 'Select a staff user' }] }]
                  const getPrimarySection = (u) => {
                    const sections =
                      Array.isArray(u.allowedSections) && u.allowedSections.length > 0
                        ? u.allowedSections
                        : u.section
                          ? [u.section]
                          : []
                  ...
                  const other =
                    noSection.length > 0
                      ? [
                          {
                            sectionLabel: 'No section assigned',
                            options: noSection.map((u) => ({
                              value: u.id,
                              label: `${u.fullName || u.email?.split('@')[0] || 'Unknown'} (${u.email})`,
                            })),
                          },
            ...
            {selectedAccessUser && (
              <>
                <div>
                  <label className="block text-[10px] font-bold text-[#5c7355] uppercase tracking-wider mb-2">
                    Allowed Sections
                  </label>
                  <p className="text-[11px] text-[#5c574f] font-medium mb-3">
                    Toggle which sections this staff can access. Staff can only use units within the selected sections.
                  </p>
                  <div className="space-y-2">
                    {SECTION_OPTIONS.map((opt) => {
                      const isChecked = accessAllowedSections.includes(opt.value)
                      return (
                        <div
                          key={opt.value}
                          className={`flex items-center justify-between gap-3 p-3 rounded-xl border-2 transition-all duration-200 cursor-pointer ${
                            isChecked ? 'bg-[#f0f5ee] border-[#1e4d2b]/40' : 'bg-white border-[#e8e0d4]'
                          }`}
                          onClick={() => toggleAccessSection(opt.value)}
                          onKeyDown={(e) => e.key === 'Enter' && toggleAccessSection(opt.value)}
                          role="button"
                          tabIndex={0}
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <span
                              className={`text-[10px] font-bold uppercase ${
                                isChecked ? 'text-[#1e4d2b]' : 'text-[#8a857c]'
                              }`}
                            >
                              {isChecked ? 'Allowed' : 'Blocked'}
                            </span>
                            <div
                              className={`relative w-11 h-6 rounded-full transition-colors duration-300 ease-out ${
                                isChecked ? 'bg-[#1e4d2b]' : 'bg-[#e8e0d4]'
                              }`}
                              aria-hidden
                            >
                              <span
                                className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow-md transition-all duration-300 ease-out ${
                                  isChecked ? 'left-1 translate-x-5' : 'left-1 translate-x-0'
                                }`}
                ...
                <button
                  type="button"
                  onClick={saveAccessSections}
                  disabled={accessSaving}
                  className="mt-2 inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-[#1e4d2b] text-white font-bold hover:bg-[#153019] disabled:opacity-60 transition-all duration-300 ease-[cubic-bezier(0.33,1,0.68,1)] hover:scale-[1.02] active:scale-[0.98] shadow-md hover:shadow-lg border border-[#1e4d2b]/20 min-h-[44px] touch-manipulation"
                >
                  {accessSaving ? 'Saving...' : 'Save Section Access'}
                </button>
              </>
            )}

            {staffUsers.length === 0 && !accessUserId && (
